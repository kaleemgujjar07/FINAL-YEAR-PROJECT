...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const cognivio_app_email = settings['cognivio_app_email'];
  const cognivio_base_url = settings['cognivio_base_url'];

  const url = checkAndCorrectURL(cognivio_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | Cognivio',
    cognivio_app_email,
