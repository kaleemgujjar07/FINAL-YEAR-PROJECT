...
const statsController = {
  revenueForecast: async (req, res) => {
    try {
      ...
      const aiResponse = await axios.post(`${AI_SERVICE_URL}/forecast/series`, { historical_data: historicalData, steps: 6 });
  ...
  getInsights: async (req, res) => {
    try {
      ...
      const revTrend = revResponse.data.trend;
      const expTrend = expResponse.data.trend;
