...
const setup = async (req, res) => {
  ...
  for (const filePath of settingsFiles) {
    ...
    const settingsToUpdate = {
      cognivio_app_email: email,
      cognivio_app_company_email: email,
      cognivio_app_timezone: timezone,
      cognivio_app_country: country,
      cognivio_app_language: language || 'en_us',
    };
  ...
  return res.status(200).json({
    success: true,
    result: {},
    message: 'Successfully Cognivio App Setup',
  });
};
