...
export const Checkout = () => {
    ...
    // Handle auto-fill from AI Assistant
    useEffect(() => {
        if (location.state?.autoAddress && addresses.length > 0) {
            const autoAddr = location.state.autoAddress;
            
            // 1. Try to find match in existing addresses
            const found = addresses.find(a => 
                a.street === autoAddr.street && 
                a.city === autoAddr.city && 
                a.postalCode === autoAddr.postalCode
            );
            
            if (found) {
                setSelectedAddress(found);
            } else {
                // 2. Pre-fill form if no match found
                reset(autoAddr);
            }
        }
    }, [location.state, addresses, reset]);
