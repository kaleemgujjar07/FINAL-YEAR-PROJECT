...
import { Box, Checkbox, Rating, Stack, Typography, useMediaQuery, Button, Paper, useTheme } from '@mui/material'
...
export const ProductDetails = () => {
  ...
  const averageRating = totalReviews === 0 ? 0 : Math.ceil(totalReviewRating / totalReviews)
  ...
  const handleIncreaseQty = () => {
    if (quantity < 20 && quantity < (product?.stockQuantity ?? 20)) {
      setQuantity(quantity + 1)
    }
  }
  ...
  // --- SLIDER (react-swipeable) ---
  const [activeStep, setActiveStep] = useState(0)
  const maxSteps = product?.images?.length || 0

  // Sync thumbnail click to slider as well
  ...
  const handleNext = () => {
    setActiveStep((prevActiveStep) => Math.min(prevActiveStep + 1, Math.max(0, maxSteps - 1)));
  };
  ...
  // keep selectedImageIndex in sync when activeStep changes (for non-slider view)
  useEffect(() => {
    setSelectedImageIndex(activeStep)
  }, [activeStep])
              ...
              <Stack width={is480 ? "auto" : is1420 ? "auto" : '88rem'} p={is480 ? 2 : 0} height={is840 ? "auto" : "50rem"} rowGap={5} mt={is840 ? 0 : 5} justifyContent={'center'} mb={5} flexDirection={is840 ? "column" : "row"} columnGap={is990 ? "2rem" : "5rem"}>
                ...
                <Stack sx={{ flexDirection: "row", columnGap: "2.5rem", alignSelf: "flex-start", height: "100%" }}>
                  ...
                  {!is1420 && <Stack sx={{ display: "flex", rowGap: '1.5rem', height: "100%", overflowY: "scroll" }}>
                    {
                      product && product.images.map((image, index) => (
                        <motion.div key={index} whileHover={{ scale: 1.1 }} whileTap={{ scale: 1 }} style={{ width: "200px", cursor: "pointer" }} onClick={() => onThumbnailClick(index)}>
                          <img 
                            ...
                            onError={(e) => {
                                ...
                                if (!image?.startsWith('/assets/')) {
                                    e.target.src = `https://picsum.photos/seed/${id}-${index}/400`;
                                }
                            }}
                          />
                        </motion.div>
                  ...
                  <Stack mt={is480 ? "0rem" : '5rem'}>
                    {
                      is1420 ?
                        <Stack width={is480 ? "100%" : is990 ? '400px' : "500px"} {...handlers} sx={{ overflow: "hidden", position: "relative" }}>
                          {/* show activeStep image */}
                          <Box 
                            component="img" 
                            src={product?.images?.[activeStep]?.startsWith('http') ? product?.images?.[activeStep] : `${process.env.REACT_APP_BASE_URL}${product?.images?.[activeStep]}`} 
                            alt={product?.title} 
                            sx={{ width: "100%", height: "100%", objectFit: "contain", overflow: "hidden", aspectRatio: "1/1" }} 
                            onError={(e) => {
                                e.target.onerror = null;
                                if (!product?.images?.[activeStep]?.startsWith('/assets/')) {
                                    e.target.src = `https://picsum.photos/seed/${id}-${activeStep}/400`;
                                }
                            }}
                          />
                        ...
                        <div style={{ width: "100%" }}>
                          <img 
                            ...
                            src={product?.images?.[selectedImageIndex]?.startsWith('http') ? product?.images?.[selectedImageIndex] : `${process.env.REACT_APP_BASE_URL}${product?.images?.[selectedImageIndex]}`} 
                            alt={`${product?.title}`} 
                            onError={(e) => {
                                e.target.onerror = null;
                                if (!product?.images?.[selectedImageIndex]?.startsWith('/assets/')) {
                                    e.target.src = `https://picsum.photos/seed/${id}-${selectedImageIndex}/400`;
                                }
                            }}
                          />
                        </div>
                        ...
                        <Stack flexDirection={'row'} columnGap={is387 ? ".5rem" : ".2rem"} >
                          {
                            COLORS.map((color, index) => (
                              <div key={index} style={{ backgroundColor: "white", border: selectedColorIndex === index ? `1px solid ${theme.palette.primary.dark}` : "", width: is340 ? "40px" : "50px", height: is340 ? "40px" : "50px", display: "flex", justifyContent: "center", alignItems: "center", borderRadius: "100%", }}>
