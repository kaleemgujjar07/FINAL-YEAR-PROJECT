  ...
  return (
    ...
    >
        ...
        <Stack sx={{ bgcolor: '#f9f9f9', borderRadius: '12px', mb: 2, overflow: 'hidden' }}>
            <img 
                width={'100%'} 
                style={{aspectRatio:1/1,objectFit:"contain", padding: '1rem'}} 
                height={'100%'}  
                src={thumbnail?.startsWith('http') ? thumbnail : `${process.env.REACT_APP_BASE_URL}${thumbnail}`} 
                alt={`${title}`} 
                onError={(e) => {
                    e.target.onerror = null; 
                    // Fallback to a better placeholder if it's not a local asset that failed
                    if (!thumbnail?.startsWith('/assets/')) {
                        e.target.src = `https://picsum.photos/seed/${id}/400`;
                    }
                }}
            />
        </Stack>
